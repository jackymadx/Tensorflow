...
for i in range(100):
    ...
    if (i+1)%25==0:
        logging.info('Step #' + str(i+1) + ' A = ' + str(sess.run(A)))
        logging.info('Loss = ' + str(sess.run(loss, feed_dict={x_data: rand_x, y_target: rand_y})))

# Evaluate accuracy (loss) on test set
mse_test = sess.run(loss, feed_dict={x_data: np.transpose([x_vals_test]), y_target: np.transpose([y_vals_test])})
mse_train = sess.run(loss, feed_dict={x_data: np.transpose([x_vals_train]), y_target: np.transpose([y_vals_train])})
logging.info('MSE on test:' + str(np.round(mse_test, 2)))
logging.info('MSE on train:' + str(np.round(mse_train, 2)))
...
for i in range(1800):
    ...
    if (i+1)%200==0:
        logging.info('Step #' + str(i+1) + ' A = ' + str(sess.run(A)))
        logging.info('Loss = ' + str(sess.run(xentropy, feed_dict={x_data: rand_x, y_target: rand_y})))
...
logging.info('Accuracy on train set: ' + str(acc_value_train))
logging.info('Accuracy on test set: ' + str(acc_value_test))
